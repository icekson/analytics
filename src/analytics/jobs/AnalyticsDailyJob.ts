    ...
    perform(params: any|AggregationJobParams): Promise<any> {
        return new Promise((resolve) => {
            try{
                ...
                if((params as AggregationJobParams).type === AggregationType.Daily){
                    task = new DailyTask('analytics_daily_stats', date);
                    task.run(async () => {
                        ...
                        let data = await this.aggregator.fetchDailyStats(task.getCurrentPartition());
                        if(data && data.length > 0){
                            this.logger.warn('perform: Daily data has already been collected for partition'+ task.getCurrentPartition().getPartitionNumber() +' for period - from: %s; to: %s', task.getCurrentPartition().dateTimeInterval().from.format('YYYY-MM-DD HH:mm:ss'), task.getCurrentPartition().dateTimeInterval().to.format('YYYY-MM-DD HH:mm:ss') )
                            return;
                        }
                        await this.aggregator.aggregateDailyStats(task.getCurrentPartition());
                    });
                    resolve();
                }
            }catch (e) {
